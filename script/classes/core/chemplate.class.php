...
class chemplate {
  ...
  public function display($file,$cache=false){
	...
	if(!class_exists("chemplate_compiler"))require "chemplate_compiler.class.php";
	$compiler=new chemplate_compiler($this);
  ...
  final protected function load_cache($file){
    $file_cache=$this->cd.str_replace(array('/','.tpl'),array('_','.html'),$file);
    $cache_exist=false;
  ...
  final public function get_cache($file){
    return str_replace(array('/','.tpl'),array('_','.html'),$file);
  }
