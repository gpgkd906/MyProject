...
abstract class core {
  ...
  private function action(){
	...
	$_page=array($this->request["app"][0],$this->request["app"][1]);
	if($this->getInstance("trigger")->isRegisterPage($_page)){
	  $this->getInstance("trigger")->callPageTrigger($_page);
	  unset($exception);
	}
	if(isset($exception)){
  ...
  public function application($map){
    ...
    $this->request=call_user_func_array(array($this->dispatcher,"map"),array($map,My::app,My::template));
    if(empty($this->request["logic"]) && empty($this->request["view"])){
      $page=array(
		     $this->request["app"][0],
		     $this->request["app"][1],
		     );
      if(!$this->getInstance("trigger")->isRegisterPage($page)){
	return false;
      }
    }
    return true;
  ...
  public function register($controller,$page,$callback){
    $this->getInstance("trigger")->registerPage(array($controller,$page),$callback);
  }
